<?php
	$lang = array(
	'about' => 'عن ',
	'access' => 'الدخول',
	'access_limits' => 'حدود الدخول',
	'account_name' => 'اسم الحساب',
	'actions' => 'الإجراءات',
	'action_confirm' => 'لا يمكن التراجع عن هذا الإجراء. هل متأكد من رغبتك في الاستمرار؟',
	'activate' => 'تفعيل',
	'active' => 'مُفعل',
	'add' => 'أضف',
	'added_by' => 'تمت الإضافة بواسطة',
	'additional_reports' => 'تقارير إضافية',
	'add_edit' => 'أضف/حرر',
	'add_field' => 'أضف حقل',
	'add_language' => 'أضف لغة',
	'add_new' => 'أضف جديد',
	'add_new_category' => 'أضف فئة جديدة',
	'add_translation' => 'أضف ترجمة',
	'admin' => 'الإدارة',
	'alerts' => 'الحصول على تنبيهات',
	'alerts_alert_me' => ':إعلمني اذا تم ملء تقرير، أو عند المحاولة',
	'alerts_btn_send' => 'حفظ التنبيهات الخاصة بى',
	'alerts_email' => ':عنوان البريد الإلكتروني',
	'alerts_enter_email' => 'أدخل عنوان البريد الإلكتروني',
	'alerts_enter_mobile' => 'أدخل رقم هاتف المحمول مع رمز الدولة',
	'alerts_get' => 'أحصل على تنبيهات',
	'alerts_mobile_phone' => ':الهاتف المحمول',
	'alerts_place_spot' => 'أو ضع العلامة في نقطة على الخريطة لتتلقي تنبيه عند إضافة تقرير على مدار 20 كم حول العلامة',
	'alerts_rss' => 'RSS Feeds (انسخ الرابط التالى)',
	'alerts_select_city' => 'إختر مدينة',
	'alerts_step1_select_city' => '<strong> :الخطوة الاولى</strong>:اختار مدينتك او موقعك',
	'alerts_step2_send_alerts' => '<strong> :الخطوة الثانية</strong>:ارسل التنبيهات إلي',
	'alert_confirm_previous' => 'أكد على طلب سابق للحصول على تنبيهات',
	'alert_saved' => 'تم حفظ التنبيه الخاص بك',
	'all' => 'الكل',
	'allowed' => 'مسموح',
	'all_categories' => 'كل التصنيفات',
	'and' => 'و',
	'api' => '(API) واجهة برمجة التطبيقات',
	'approve' => 'وافق',
	'approved' => 'تمت الموافقة',
	'approved_reports' => 'التقارير التى تم الموافقة عليها',
	'approve_this_report' => 'الموافقة على هذا التقرير',
	'approximate' => 'تقريبى',
	'archive' => 'أرشفة',
	'archived' => 'تمت الأرشفة',
	'at' => 'على',
	'author' => 'المحرر',
	'awaiting_approval' => 'في انتظار الموافقة',
	'awaiting_verification' => 'فى انتظار التحقق من المصداقية',
	'blog' => 'مدونة',
	'cancel' => 'إلغاء',
	'categories' => 'التصنيفات',
	'category' => 'التصنيف',
	'category_filter' => 'تحديد التصنيفات',
	'category_has_been' => 'The category has been ',
	'category_name' => 'أسم التصنيف',
	'choose' => 'قم باختيار',
	'choose_data_points' => 'إختر بيانات محددة للتحميل',
	'choose_field_type' => 'إختر نوع الحقل',
	'cleanurl' => 'Clean URLs',
	'close' => 'إغلاق',
	'clusters' => 'المجموعات',
	'color' => 'اللون',
	'comment' => 'التعليق',
	'comments' => 'التعليقات',
	'comment_details' => 'تفاصيل التعليق',
	'comment_rating' => 'رتبة التعليق',
	'configuration_saved' => '!تم حفظ الإعدادات الخاصة بك',
	'configure' => 'ضبط',
	'contact' => 'اتصل بنا',
	'copyright' => '© 2009 Ushahidi.com حقوق النسخ. جميع الحقوق محفوظة',
	'create' => 'إنشاء',
	'create_edit' => 'أنشئ/حرر',
	'create_new' => 'أنشئ جديد',
	'create_report' => 'إنشاء تقرير',
	'credibility' => 'المصداقية',
	'data' => 'معلومات',
	'date' => 'تاريخ',
	'date_format' => '(سنة/يوم/شهر)',
	'date_time' => 'التاريخ والوقت',
	'day' => 'اليوم',
	'deactivate' => 'عطل',
	'default_location_name' => 'نيروبى، كينيا',
	'delete' => 'حذف',
	'deleted' => 'تم الحذف',
	'deletes' => 'المحذوفات',
	'delete_disabled' => 'تم تعطيل الحذف',
	'delete_report' => 'إحذف هذا التقرير',
	'delete_spam' => 'إحذف السخام',
	'demo' => 'تجريبى',
	'description' => 'وصف',
	'detailed_location_example' => 'مثال: زاوية سوق المدينة، الشارع الخامس الجادة الرابعة، جوهانسبرج',
	'details' => 'تفاصيل',
	'direct_report' => 'تقرير مباشر',
	'disabled' => 'تم التعطيل',
	'disapprove' => 'لم تتم الموافقة',
	'download_reports' => 'تحميل التقارير',
	'edit' => 'حرر',
	'edit_form_fields' => 'حرر حقول الاستمارة',
	'edit_report' => 'حرر التقرير',
	'email' => 'البريد الإلكتروني',
	'email_address' => 'عنوان البريد الإلكتروني',
	'email_configuration' => 'إعدادات خادم البريد',
	'email_server_host' => 'مضيف خادم البريد',
	'email_server_password' => 'كلمة المرور لخادم البريد',
	'email_server_port' => 'منفذ خادم البريد',
	'email_server_ssl_support' => 'الخادوم يدعم طبقة المقابس الآمنة',
	'email_server_type' => 'نوع خادم البريد',
	'email_server_username' => 'اسم المستخدم لخادم البريد',
	'email_settings_comment_0' => 'يجب ان تكون الإعدادات مرتبطة بهذا البريد الإلكتروني',
	'email_settings_comment_00' => 'لتتمكن من استقبال التقارير عبر البريد، أدخل إعدادات حساب البريد الإلكتروني. برجاء العلم أنه سيتم تلقي الرسائل علي البريد الخاص بالموقع',
	'email_settings_comment_1' => 'بعض الخوادم تتطلب عنوان البريد الإلكتروني بالكامل',
	'email_settings_comment_2' => 'كلمة السر لحساب البريد الإلكتروني',
	'email_settings_comment_3' => 'Common ports: 25, 110, 995 (Gmail POP3 SSL), 993 (Gmail IMAP SSL)',
	'email_settings_comment_4' => 'Examples: mail.yourwebsite.com, imap.gmail.com, pop.gmail.com',
	'email_settings_comment_5' => 'Examples: pop3, imap',
	'email_settings_comment_6' => 'SSL تفعيل أو تعطيل الإتصال الآمن
',
	'end_point' => 'إلي',
	'error' => '!خطأ',
	'example' => 'مثال',
	'example_country' => 'كينيا',
	'feed' => 'التغذية',
	'feedback' => 'شاركنا رأيك',
	'feedback_reports' => '.برجاء إعلامنا برأيك من خلال خبراتك بالنقر على الزر أدناه',
	'feeds' => 'التغذية',
	'feed_has_been' => 'هذا الرابط تم',
	'feed_items' => 'موضوعات للتغذية',
	'feed_name' => 'اسم التغذية',
	'feed_url' => 'رابط التغذية',
	'field_unused' => 'حقول غير مستخدمة',
	'file' => 'ملف',
	'filters' => 'تصفيات',
	'find' => 'ابحث عن',
	'find_location' => 'ابحث عن مكان',
	'first_name' => 'الاسم الأول',
	'forgot_password' => 'لا تتذكر كلمة المرور الخاصة بك؟',
	'form' => 'استمارة',
	'forms' => 'استمارات',
	'form_description' => 'وصف الاستمارة',
	'form_edit' => 'حرر هذه الاستمارة',
	'form_has_been' => 'The form has been',
	'form_title' => 'عنوان الاستمارة',
	'from' => 'من',
	'full_name' => 'الاسم الكامل',
	'geolocation_available' => 'الموقع الجغرافى متاح',
	'hashtag' => 'الوسم',
	'has_been' => 'Has Been',
	'help' => 'كيفية المساعدة',
	'hide' => 'إخفاء',
	'hide_this_message' => 'أخفى هذه الرسالة',
	'home' => 'الرئيسية',
	'how_to_report' => 'كيفية الإبلاغ',
	'id' => 'الهوية',
	'image' => 'صورة',
	'images' => 'صور',
	'image_icon' => 'صورة/أيقونة',
	'inactive' => 'غير فعَال',
	'inbox' => 'البريد الوارد',
	'incident' => 'حدث',
	'incidents_listed' => '<span>(من على الخريطة، مدرجة بترتيب زمنى)</span> الأحداث',
	'incidents_nearby' => 'أحداث قريبة من موقعك',
	'incident_location' => 'مكان الحدث',
	'include' => 'إدرج',
	'include_categories' => 'إدرج فئات',
	'include_description' => 'إدرج توصيف',
	'include_detail' => 'إدرج تفاصيل على قدر الامكان',
	'include_latitude' => 'إدرج خط عرض',
	'include_location_information' => 'إدراج معلومات عن المكان',
	'include_longitude' => 'إدرج خط طول',
	'incoming_media' => 'مواد إعلامية واردة',
	'information_evaluation' => 'تقييم المعلومات',
	'input_location' => 'أدخل موقعك بالتحديد',
	'in_response_to' => 'استجابةً لـ',
	'ip_address' => 'عنوان بروتوكول الانترنت',
	'item' => 'بند',
	'items' => 'بنود',
	'item_details' => 'تفاصيل البند',
	'item_title' => 'عنوان البند',
	'key' => 'مفتاح',
	'kml_kmz_file' => 'KMZ/KML File',
	'kml_kmz_upload' => 'Upload KMZ/KML File',
	'kml_url' => 'KML URL',
	'laconica' => 'Laconica',
	'language' => 'اللغة',
	'last' => 'أخير',
	'last_month' => 'الشهر الأخير',
	'last_name' => 'الاسم الأخير',
	'last_year' => 'السنة الأخيرة',
	'latitude' => 'خط العرض',
	'layers' => 'الطبقات',
	'layers_filter' => 'طبقات أخرى',
	'layer_has_been' => 'The Layer Has Been',
	'layer_name' => 'اسم الطبقة',
	'layer_url' => 'Layer URL',
	'level' => 'مستوى',
	'level_has_been' => 'This level has been',
	'level_name' => 'اسم المستوى',
	'limited' => 'محدود',
	'link' => 'الرابط',
	'location' => 'المكان',
	'locations' => 'الأماكن',
	'location_example' => 'المدينة، المحافظة/الدولة',
	'login' => 'الولوج',
	'longitude' => 'خطل الطول',
	'map' => 'خريطة',
	'mark_read' => 'إجعلها مقروءة',
	'mark_unread' => 'إجعلها غير مقروءة',
	'media' => 'مواد إعلامية',
	'media_filter' => 'تصفية المواد الإعلامية',
	'message' => 'رسالة',
	'messages' => 'الرسائل',
	'message_details' => 'تفاصيل الرسالة',
	'mobile' => 'الهاتف المحمول',
	'modify' => 'تعديل',
	'modify_date' => 'تعديل تاريخ',
	'month' => 'شهر',
	'more' => 'المزيد',
	'multi_country_instance' => 'هل هذه الخريطة تشمل دول متعددة؟',
	'name' => 'الاسم',
	'nearby_report' => 'أحداث قريبة',
	'new' => 'جديد',
	'news' => 'أخبار',
	'news_feeds' => 'التغذية الإخبارية',
	'news_source' => 'مصدر الأخبار',
	'new_category' => 'تصنيف جديدة',
	'new_password' => 'كلمة سر جديدة',
	'new_report' => 'تقرير جديد',
	'next' => 'التالي',
	'no' => 'لا',
	'notices' => 'إشعارات',
	'not_approved' => 'لم تتم الموافقة عليه',
	'not_approved_singular' => 'لم تتم الموافقة عليه',
	'not_spam' => 'ليس سخام',
	'not_specified' => 'غير محدد',
	'no_results' => 'لايوجد نتائج',
	'official_news' => 'أخبار رسمية وعامة',
	'option' => 'خيار',
	'optional' => 'اختياري',
	'options' => 'خيارات',
	'organization' => 'منظمة',
	'organizations' => 'منظمات',
	'organization_description' => 'وصف المنظمة',
	'organization_email' => 'البريد الإلكتروني للمنظمة',
	'organization_has_been' => 'The Organization Has Been',
	'organization_name' => 'اسم المنظمة',
	'organization_phone_1' => 'تليفون المنظمة 1',
	'organization_phone_2' => 'تليفون المنظمة 2',
	'organization_website' => 'الموقع الإلكتروني للمنظمة',
	'original' => 'الأصل',
	'original_description' => 'الوصف الأصلي',
	'original_title' => 'العنوان الأصلي',
	'other_ushahidi_instances' => 'تطبيقات يوشاهيدي أخرى',
	'outbox' => 'البريد الصادر',
	'outgoing' => 'البريد الخارج',
	'page' => 'صفحة',
	'pages' => 'الصفحات',
	'page_description' => 'وصف الصفحة',
	'page_has_been' => 'The Page Has Been',
	'page_tab_name' => 'اسم التبويب للصفحة',
	'page_title' => 'عنوان الصفحة',
	'parent_category' => 'الصنيف الرئيسي',
	'password' => 'كلمة السر',
	'password_again' => 'أكتب كلمة المرور مرة أخرى',
	'password_forgot' => 'لا تتذكر كلمة المرور؟',
	'password_reset_confirm' => 'إطلع على بريدك الإلكتروني لمعرفة كلمة المرور الجديدة',
	'password_reset_prompt' => 'ادخل عنوان البريد الإلكتروني المستحدم في التسجيل',
	'password_save' => 'ابقى متصلاً من على هذا الحاسب',
	'past_month' => 'الشهر الماضي',
	'past_year' => 'العام الماضي',
	'pending' => 'قيد الإرسال',
	'per' => 'لكل',
	'personal_information' => '<span>اختياري</span> معلومات شخصية',
	'phone' => 'تليفون',
	'pictures' => 'صور',
	'pictures_and_videos' => 'صور وفيديو',
	'pinpoint_location' => '*ابحث عن مكانك بتحديد إحداثيات الطول والعرض أو أنقر على الخريطة لتحديد المكان',
	'play' => 'تشغيل',
	'please_note' => 'برجاء الملاحظة',
	'plugins' => 'برامج مساعدة',
	'plugin_url' => 'البرامج المساعدة للموقع الإلكتروني',
	'preview' => 'معاينة',
	'preview_item' => 'معاينة الموضوع',
	'preview_message' => 'معاينة الرسالة',
	'previous' => 'السابق',
	'profile_saved' => 'تم حفظة ملفك الشخصى',
	'quick_stats' => 'إحصائيات سريعة',
	'rating' => 'تصنيف',
	'read' => 'Le�do',
	'receive' => 'استقبل',
	'receive_from' => 'استقبل من',
	'receive_notifications' => 'استقبل إشعارات',
	'recent_reports' => 'أحدث التقارير',
	'refresh_news_feeds' => 'إنعاش التغذية الإخبارية',
	'remove' => 'إحذف',
	'reply' => 'الرد',
	'report' => 'تقرير',
	'reporter' => 'المحرر',
	'reporters' => 'المحررون',
	'reporter_date' => 'تاريخ التحرير',
	'reporter_email' => 'البريد الإلكترونى لمحرر التقرير',
	'reporter_first_name' => 'الاسم الأول للمحرر التقرير',
	'reporter_has_been' => 'The Reporter Has Been',
	'reporter_ip' => 'عنوان بروتوكول الانترنت لمحرر التقرير',
	'reporter_last_name' => 'الاسم الأخير لمحرر التقرير',
	'reporter_level' => 'مستوى محرر التقرير',
	'reporter_levels' => 'مستويات محرري التقارير',
	'reporter_phone' => 'تليفون محرر التقرير',
	'reports' => 'التقارير',
	'reports_avg_day' => 'متوسط التقرير اليومية',
	'reports_btn_browse' => 'تصفح',
	'reports_btn_submit' => 'تقديم',
	'reports_categories' => 'فئات',
	'reports_date' => 'تاريخ',
	'reports_description' => 'وصف',
	'reports_download_csv' => 'CSV سيتم تحميل التقارير في صيغة',
	'reports_email' => 'البريد الإلكتروني',
	'reports_find_location' => 'أعثر علي أقرب مكان لك',
	'reports_first' => 'الاسم الأول',
	'reports_last' => 'الاسم الأخير',
	'reports_location_name' => 'تفصيل اسم المكان',
	'reports_news' => 'رابط مصدر الأخبار',
	'reports_optional' => 'معلومات اختيارية',
	'reports_photos' => 'رفع صور',
	'reports_return' => 'العودة لصفحة التقرير',
	'reports_select_city' => 'إختر مكان',
	'reports_submitted' => 'تم إرسال التقرير إلي فريقنا للمعانية. سنتصل بك في حال وجود ضرورة',
	'reports_submit_new' => 'إنشاء تقرير جديد',
	'reports_time' => 'الوقت',
	'reports_timeline' => 'الخط الزمني',
	'reports_title' => 'عنوان التقرير',
	'reports_total' => 'مجمل التقارير',
	'reports_video' => 'رابط الفيديو',
	'report_an_incident' => 'إصدر تقرير عن حدث',
	'report_details' => 'تفاصيل التقارير',
	'report_edit_dropdown_1_default' => '--- إختر واحداً ---',
	'report_edit_dropdown_1_item_1' => '(نعم، المصدر ذات صلة مباشرة بالمعلومة (شاهد أو فاعل',
	'report_edit_dropdown_1_item_2' => 'نعم، المصدر ذات صلة بالمعلومة، لكن قد يكون مخطئ',
	'report_edit_dropdown_1_item_3' => 'نعم، المصدر ليس على صلة مباشرة بالمعلومة، لكنه عادة على صواب
',
	'report_edit_dropdown_1_item_4' => 'ليس دائما، لكن فى العادة على صواب',
	'report_edit_dropdown_1_item_5' => 'لا، المصدر ليس على صلة مباشرة بالمعلومة، ولم يكن',
	'report_edit_dropdown_1_item_6' => 'لا أعلم',
	'report_edit_dropdown_1_title' => 'مصداقية المصدر',
	'report_edit_dropdown_2_default' => '--- إختر واحداً ---',
	'report_edit_dropdown_2_item_1' => 'نعم، تم تأكيد المعلومة بواسطة مصادر متعددة مستقلة',
	'report_edit_dropdown_2_item_2' => 'نعم، لم يتم تأكيد المعلومة بعد، لكن مرجح أن تكون صحيحة',
	'report_edit_dropdown_2_item_3' => 'نعم، المعلومة منطقية',
	'report_edit_dropdown_2_item_4' => 'لا، المعلومة مفاجأة',
	'report_edit_dropdown_2_item_5' => 'لا، المعلومة غير مرجحة وقد تكون مضللة',
	'report_edit_dropdown_2_item_6' => 'لا أعلم',
	'report_edit_dropdown_2_title' => 'إحتمالية المعلومة',
	'report_option_1' => 'من خلال إرسال رسالة إلى',
	'report_option_2' => 'بإرسال بريد إلكتروني إلى ',
	'report_option_3' => 'من خلال رسالة تويتر بالوسوم التالية',
	'report_option_4' => 'من خلال ملء هذه الاستمارة',
	'report_saved' => 'تم حفظ التقرير الخاص بك',
	'report_title' => 'عنوان التقرير',
	'request_information' => 'طلب الحصول على مزيد من المعلومات',
	'request_location' => 'طلب بالمكان',
	'requirements' => 'متطلبات',
	'reset_password' => 'إعادة تعيين كلمة السر',
	'retrieve_city_names' => 'الحصول على أسماء المدن للدولة المُختارة',
	'role' => 'دور',
	'rss' => 'RSS',
	'save' => 'حفظ',
	'saved' => 'تم الحفظ',
	'save_close' => 'حفظ وإغلاق',
	'save_report' => 'حفظ التقرير',
	'schedule' => 'الجدول',
	'scheduler' => 'جدولة',
	'scheduler_day' => 'يوم',
	'scheduler_hour' => 'ساعة',
	'scheduler_log' => 'سجل جدولة الأعمال',
	'scheduler_minute' => 'المحضر',
	'scheduler_weekday' => 'يوم في الأسبوع',
	'search' => 'بحث',
	'search_results' => 'نتائج البحث',
	'security_code' => 'الرمز الأمنى',
	'select_all' => 'اختر الكل',
	'select_field_type' => 'اختر نوع الحقل',
	'select_form_type' => 'اختر نوع الاستمارة',
	'select_in_map' => 'حدد فى الخريطة',
	'select_multiple' => 'اختر قدر الحاجة',
	'select_one' => 'رجاء التأكيد أنك قمت باختيار بند',
	'select_theme' => 'اختر شكل',
	'send' => 'إرسل',
	'sending_to' => 'الإرسال إلى',
	'sent' => 'تم الإرسال',
	'sent_by' => 'تم الإرسال بواسطة',
	'server_address' => 'عنوان الخادم',
	'server_type' => 'نوع الخادم',
	'service' => 'الخدمة',
	'service_username' => 'اسم مستخدم الخدمة',
	'service_user_id' => 'هوية مستخدم الخدمة',
	'share' => 'مشاركة',
	'shared_data' => 'بيانات تم تشاركها',
	'share_has_been' => 'The share has been',
	'sharing' => 'يتم المشاركة',
	'show' => 'أظهر',
	'show_all' => 'أظهر الكل',
	'site' => 'الموقع الإلكتروني',
	'site_email_address' => 'عنوان البريد الإلكتروني للموقع',
	'site_url' => 'رابط الموقع الإلكتروني',
	'sms' => 'رسال نصية قصيرة',
	'source' => 'المصدر',
	'source_name' => 'اسم المصدر',
	'source_url' => 'رابط المصدر',
	'spam' => 'سخام',
	'ssl_support' => 'SSL Support?',
	'start_point' => 'من',
	'step' => 'خطوة',
	'submit' => 'إنشاء تقرير',
	'submitted_by' => 'تم الإنشاء بواسطة',
	'submit_sms' => 'الإبلاغ بواسطة رسالة نصية قصيرة',
	'submit_sms1' => 'إرسل الرسالة النصية الخاصة بك إلى',
	'submit_sms2' => 'على تليفونك',
	'success' => '!نجحت',
	'successfuly_imported' => 'تم الاستيراد بنجاح',
	'surname' => 'اسم العائلة',
	'system' => 'نظام',
	'tcp_port' => 'TCP Port',
	'themes' => 'أشكال',
	'theme_default' => 'الشكل الافتراضي ليوشاهيدي',
	'theme_settings' => 'إعدادات الشكل',
	'this' => 'هذا',
	'this_day' => 'اليوم',
	'this_month' => 'هذا الشهر',
	'this_year' => 'هذا العام',
	'time' => 'الوقت',
	'title' => 'عنوان ',
	'to' => 'إلى',
	'today' => 'اليوم',
	'today_at' => 'اليوم على',
	'translated' => 'تمت الترجمة',
	'translated_description' => 'الوصف المترجم',
	'translated_title' => 'العنوان المترجم',
	'translate_to' => 'الترجمة إلى',
	'translation' => 'الترجمة',
	'translation_saved' => 'تم حفظ الترجمة',
	'trustworthiness' => 'مصداقية',
	'trustworthiness_option_1' => '(نعم، المصدر ذات صلة مباشرة بالمعلومة (شاهد أو فاعل',
	'trustworthiness_option_2' => 'نعم، المصدر ذات صلة بالمعلومة، لكن قد يكون مخطئ',
	'trustworthiness_option_3' => 'نعم، المصدر ليس على صلة مباشرة بالمعلومة، لكنه عادة على صواب
',
	'trustworthiness_option_4' => 'ليس دائما، لكن فى العادة على صواب',
	'trustworthiness_option_5' => 'لا، المصدر ليس على صلة مباشرة بالمعلومة، ولم يكن',
	'twitter' => 'تويتر',
	'unread' => 'غير مقروء',
	'unverified' => 'لم يتم التحقق',
	'update_feeds' => 'حدث التغذية',
	'upload' => 'رفع',
	'upload_file' => 'الملف للرفع',
	'upload_reports' => 'رفع التقارير',
	'upload_reports_detail_1' => 'بالاستمارة أدناه يمكنك استيراد أحداث على محرك يوشاهيدي',
	'upload_reports_detail_2' => 'CSV يجب أن تكون التقارير في صيغة',
	'upload_reports_detail_3' => 'في حالة تكرار رقم التقرير في قاعدة البيانات، سيتم تجاهل الملف بالكامل',
	'upload_reports_detail_4' => 'يجب علي الأقل وجود عنوان الحدث والتاريخ',
	'upload_reports_detail_5' => 'CSV عينة من تقرير في صيغة',
	'upload_reports_detail_6' => '#,INCIDENT TITLE,INCIDENT DATE,LOCATION,DESCRIPTION,CATEGORY,APPROVED,VERIFIED<br />"1","Suspected death in Nairobi","2009-05-15 01:06:00","Nairobi","Three cases have been confirmed in C. del Uruguay","DEATHS, CIVILIANS, ",YES,YES',
	'upload_reports_detail_7' => '"2","Looting","2009-03-18 10:10:00","Accra","Looting happening everywhere","RIOTS, DEATHS, PROPERTY LOSS, ",YES,NO',
	'upload_successful' => 'نجح الرفع',
	'user' => 'المستخدم',
	'username' => 'اسم المستخدم',
	'ushahidi_admin' => 'مدير يوشاهيدي',
	'verified' => 'تم التحقق منه',
	'verified_reports' => 'تقارير تم التحقق منها',
	'verify' => 'تحقق',
	'verify_this_report' => 'تحقق من هذا التقرير',
	'version' => 'إصدار',
	'via' => 'بواسطة',
	'video' => 'فيديو',
	'view' => 'عرض',
	'views' => 'عدد المشاهدين',
	'view_all' => 'عرض الكل',
	'view_all_feeds' => 'عرض كل التغذيات',
	'view_all_reports' => 'عرض كل التقارير',
	'view_items' => 'عرض البنود',
	'view_more' => 'عرض المزيد',
	'view_report' => 'عرض تقرير',
	'view_reports' => 'عرض التقارير',
	'visible' => 'مرئي',
	'waiting_approval' => 'في انتظر الموافقة',
	'waiting_verification' => 'في انتظر التحقق منه',
	'weight' => 'وزن',
	'yes' => 'نعم',
	'yesterday' => 'الأمس',
	);
?>
